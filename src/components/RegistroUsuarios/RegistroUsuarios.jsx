...
const RegistroUsuarios = () => {
    ...
    const obtenerUsuarios = async () => {
      try {
        const response = await fetch("http://15.229.116.114:3000/usuarios", {
          headers: {
    ...
    const eliminarUsuario = async (id) => {
      try {
        const response = await fetch(`http://15.229.116.114:3000/usuarios/${id}`, {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${sesion.token}`
  ...
  const validarNumeroDeTelefono = (numero) =>  numero.length == 10
    ...
    const onSubmitModificacion = async (e, usuario)=>{
      ...
      try {
        const response = await fetch(`http://15.229.116.114:3000/usuarios/${usuario.id_usuario}`, {
            method: "PUT",
            headers: {
                Authorization: `Bearer ${sesion.token}`,
                "Content-Type": "application/json",
